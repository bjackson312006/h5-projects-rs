...
mod alias {
    ...
    pub type Cache = super::cache::CacheData<{ ADBMS6830B_NUM_CHIPS }>;
}

/// Guy in charge of the segments.
struct Segments {
    service: &'static mut alias::Service,
}
impl Segments {
    ...
    fn new(r_linea: crate::SegmentIsoSpiLineAResources, r_lineb: crate::SegmentIsoSpiLineBResources) -> Self {
        use embedded_hal_bus::spi::ExclusiveDevice;
        use embassy_time::{Delay};
...
/// Lets you read the current Cache.
pub fn cache() -> &'static alias::Cache { &cache::CACHE }